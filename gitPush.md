...
Viewed gitPush.md:150-187

작업을 하고 코드가 수정되거나 새로운 파일이 추가되었을 때, GitHub에 업데이트(업로드)하는 일상적인 Git 작업 흐름(3단계 명령어)입니다.

---

## 💻 일상적인 GitHub 업데이트 (Push) 흐름

### Step 1. 변경된 상태 확인 (선택 사항)
본인이 어떤 파일들을 수정했는지 터미널에서 상태를 확인합니다.
```bash
git status
```
*(수정된 파일은 빨간색으로 표시됩니다.)*

### Step 2. 수정된 파일 추가 (Staging)
`.gitignore` 규칙을 거쳐 제외된 파일들을 빼고, 작업한 변경 사항들을 전송 후보에 등록합니다.
```bash
# 수정된 모든 파일을 한 번에 추가할 때
git add .

# 또는, 특정 수정 파일만 골라서 추가할 때 bbb
git add core/api/jobs.py
```

### Step 3. 커밋 생성 (로컬 기록 작성)
변경 내용에 대한 간단한 한글/영문 요약 메시지를 작성하여 로컬 이력에 저장합니다.
```bash
git commit -m "수정: 이력서 및 커버레터 유튜브 파이프라인 내용 추가"
```

### Step 4. GitHub에 전송 (Push)
로컬에 기록된 커밋을 GitHub 원격 서버에 동기화(업로드)합니다.
```bash
git push origin main
```
...
## 💡 유용한 Git 단축 명령 (이미 원격 주소가 연동된 상태)
이미 이전에 `git push -u origin main`을 통해 기본 업스트림 주소를 매핑해 두었기 때문에, 4단계에서는 단순히 `git push`만 입력해도 자동으로 `main` 브랜치로 코드가 올라갑니다:
```bash
git push
```
